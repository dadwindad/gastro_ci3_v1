async function fetchData(table) {
	...
	jSondata.forEach(function (params) {
		let image = new Image();
		let zone = params.dish_id.substring(0, 3);
		let subid = params.dish_id.substring(3, 7);
		let path = zone + "/" + subid;

		try {

			//find first image
			let img = jSonFolder[zone + "/"][subid + "/"]["dish/"][0];
			image.src = `/mobile/images/map/${path}/dish/${img}`;
		...
		var data = ["",path,params.dish_name_thai, params.dish_detail,params.dish_id];
		point = [...point, data];
		// console.log(point);
	});
...
async function genDataTable() {
	try {
		const dataDT = await fetchData("dish");
		// console.log(dataDT);
					...
					var dt_filter = dt_filter_table.DataTable({
						...
						fnDrawCallback: function () {
							...
							$(".editor-edit").on("click", function (e) {
								...
								$.ajax({
									url: `/maps/dish/find_last_id/${data[4]}`,
									type: "GET",
									success: function (result) {
										console.log("result : " + result);
										if (result[0].dish_id) {
											let dish_id = parseInt(result[0].dish_id.substring(1, 3))
											 $("#ed_dish_zone").val(dish_id); 
										} 
								...
								$("#ed_dish_name_thai").val(data[2]); //dish_name_thai
								$("#ed_dish_detail").val(data[3]); //dish_detail
								$("#ed_dish_id").val(data[4]); //dish_id
...
$("#dish_zone").change(function (e) {
	...
	if (val !== "#") {
		...
		$.ajax({
			url: `/maps/dish/find_last_id/${val}`,
			type: "GET",
			beforeSend: function () {
	...
	$.ajax({
		...
		success: function (params) {
			...
			var data = [
			 "",
			 "",
			 params[1].dish_name_thai,
			 params[1].dish_detail,
			 params[1].dish_id
			]
